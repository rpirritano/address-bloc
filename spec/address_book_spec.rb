...
RSpec.describe AddressBook do
  ...
  describe "attributes" do
    ...
    it "initalizes entries as empty" do
      expect(book.entries.size).to eq(0)
    end
  end
...
# Test that AddressBook's .import_from_csv() method is working as expected
  ...
  describe "remove_entry" do
    it "removes an entry using the name, phone_number and email address" do
      ...
      book.add_entry('Rob Pirritano', '011.654.2365', 'rob.def@lovelace.com')
      ...
      expect(book.entries.size).to eq 1
      expect(book.entries.first.name).to eq('Rob Pirritano')
    end
    #Test the binary_search method
    describe "#binary_search" do
      it "searches AddreessBook for a non-existant entry" do
        book.import_from_csv("entries.csv")
